...
class Command(BaseCommand):
    help = "Disable (deactivate) a student or staff account by identifier and log the action"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(
                f"Account disabled for {account_type} {identifier}"
            ))

            # Log the disable action
            logs_dir = os.path.join("logs")
            os.makedirs(logs_dir, exist_ok=True)
            log_file = os.path.join(logs_dir, "account_disables.log")

            with open(log_file, "a") as f:
                f.write(f"[{datetime.now()}] Disabled {account_type} account {identifier}\n")

        except Exception as e:
            raise CommandError(f"Error disabling account: {e}")
